...
=][]
    ...
    def __init__(self, formula_like, data={}):
        ...
        super().__init__(formula_like, data=None, matrix_type=1, solver_method="ols",
        ...
        self.variance_covariance_residual_matrix = np.matrix(self.model_data["mse"] * (I - self.model_data["H"]))
        ...
        self.model_data["test_stat_p_values"] = np.array([
            float((scipy.stats.t.sf(np.abs(t),self.model_data["degrees_of_freedom_residual"]) * 2).item()) for t in self.model_data["test_stat"]
        ])
