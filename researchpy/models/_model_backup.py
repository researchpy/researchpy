    ...
    def __fit_model(self):
        ...
        try:
            ...
            if result.success:
                ...
                self.logL_null = []
                IV_null = np.ones((self.n, 1))
                #self.IV[:, 0].reshape((self.nobs, 1))
                null_result = scipy_minimize(
                    fun=self._neg_log_likelihood,
                    x0=np.zeros(1),  # Initial guess for intercept
                    args=(IV_null, self.DV),  # Only intercept
                    jac=self._gradient_neg_log_likelihood,
                    method=self.solver_options["algorithm"],
                    options={
                        'maxiter': self.solver_options["max_iter"],
                ...
                self.LL_null.append(-null_result.fun)  # Negative log-likelihood

                # LR Chi-Squared
                self.LR_chi2 = -2 * ((-null_result.fun) - (-result.fun))
                self.model_df = self.k - 1  # Degrees of freedom (excluding intercept)
                self.model_p_value = chi2.sf(self.LR_chi2, self.model_df)

                print(f"LR Chi^2: {self.LR_chi2:.4f}, df: {self.model_df}, p-value: {self.model_p_value:.4g}")
